...
var splitext = function(filename) {
  return path.join(path.basename(filename, path.extname(filename)));
};
...
var entryFiles = function(base, keyGenerator, valueGenerator) {
  keyGenerator = keyGenerator || function(filename) {
    return splitext(filename);
  };
  valueGenerator = valueGenerator || function(filename) {
    return splitext(filename);
  };
  return walk(base)
    .filter(function(f) {
